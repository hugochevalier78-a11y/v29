...
function baseHeaders() {
  return {
    'Access-Control-Allow-Origin': process.env.ALLOWED_ORIGIN || 'https://vestiaire.pro',
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!['trial','expired','paid','blocked'].includes(status)) throw new Error('Statut invalide.');

    const patch = { status };
    if (status === 'trial') {
      patch.trial_start = new Date().toISOString();
      patch.trial_end = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
    }

    const updated = await rest(`profiles?id=eq.${encodeURIComponent(userId)}`, {
      method: 'PATCH',
      body: JSON.stringify(patch)
    });

    return { statusCode: 200, headers, body: safeJson({ ok: true, profile: updated && updated[0] }) };
  } catch (err) {
    return { statusCode: 403, headers, body: safeJson({ error: err.message || 'Erreur statut.' }) };
  }
};
