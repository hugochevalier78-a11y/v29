...
exports.handler = async (event) => {
  ...
  try {
    ...
    const users = await rest('profiles?select=id,email,nom,status,trial_start,trial_end,last_login,is_admin,created_at&order=created_at.desc');
    return { statusCode: 200, headers, body: safeJson({ users: users || [] }) };
  } catch (err) {
    return { statusCode: 403, headers, body: safeJson({ error: err.message || 'Erreur admin.' }) };
  }
};
